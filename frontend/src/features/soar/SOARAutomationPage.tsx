...
export function SOARAutomationPage() {
  ...
  const statusChartOption = useMemo(() => {
    ...
    return {
      ...
      series: [{ data: entries.map(([, v]) => v), type: 'bar', itemStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: '#06b6d4' }, { offset: 1, color: '#0891b2' }]) } }],
    }
  }, [stats])
    ...
    <div className="space-y-6">
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`flex items-center gap-2 px-4 py-2 text-sm font-medium transition-colors ${tab === t.id ? 'border-b-2 border-cyan-500 text-cyan-400' : 'text-gray-400 hover:text-gray-200'}`}
            >
      ...
      {tab === 'playbooks' && (
        <div className="space-y-4">
          <div className="flex justify-end gap-2">
            <label className="flex cursor-pointer items-center gap-2 rounded-md border border-gray-700 bg-gray-800 px-4 py-2 text-sm text-white hover:bg-gray-700">
            ...
            <button onClick={openCreate} className="flex items-center gap-2 rounded-md bg-cyan-600 px-4 py-2 text-sm text-white hover:bg-cyan-500">
          ...
          {playbooksLoading ? (
            ...
            <div className="grid gap-4">
              {playbooks.map((pb) => (
                <motion.div key={pb.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-lg border border-gray-800 bg-soc-panel p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="font-semibold text-white">{pb.name}</h4>
                        {pb.is_builtin && <span className="rounded-full bg-cyan-500/10 px-2 py-0.5 text-xs text-cyan-400">Built-in</span>}
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {pb.actions.map((a, idx) => (
                          <span key={idx} className="rounded-full bg-gray-800 px-2 py-0.5 text-xs text-cyan-400">{a.action}</span>
                        ))}
                        {pb.nodes.map((n) => (
                          <span key={n.id} className="rounded-full bg-gray-800 px-2 py-0.5 text-xs text-emerald-400">{n.type}</span>
                        ))}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      }} className="rounded-md border border-gray-700 bg-gray-800 px-3 py-1.5 text-xs text-white hover:bg-gray-700" title="Export">Export</button>
                      <button onClick={() => openEdit(pb)} className="rounded-md border border-gray-700 bg-gray-800 px-3 py-1.5 text-xs text-white hover:bg-gray-700">Edit</button>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-3 lg:col-span-1">
            ...
            {executions.map((exec: PlaybookExecution) => (
              <button key={exec.id} onClick={() => setSelectedExecution(exec)} className={`w-full rounded-md border p-3 text-left transition ${selectedExecution?.id === exec.id ? 'border-cyan-500 bg-cyan-900/20' : 'border-gray-800 bg-gray-900/50 hover:bg-gray-900'}`}>
              ...
              <>
                ...
                <div className="rounded-lg border border-gray-800 bg-soc-panel p-4">
                  ...
                  <div className="mt-2 space-y-2">
                    {(timeline || []).map((ev) => (
                      <div key={ev.id} className="flex gap-3 border-l-2 border-cyan-500/30 pl-3 text-sm">
                    ...
                    <div className="mt-2 space-y-2">
                      {(evidence || []).map((ev) => (
                        <div key={ev.id} className="rounded-md bg-gray-950 p-2 text-xs">
                          <span className="font-medium text-cyan-400">{ev.evidence_type}</span>
          ...
          {approvals.length === 0 ? (
            ...
            <div className="grid gap-4">
              {approvals.map((ap: WorkflowApproval) => (
                <motion.div key={ap.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-lg border border-gray-800 bg-soc-panel p-4">
                  <div className="flex items-start justify-between">
                    <div>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-xl border border-gray-700 bg-gray-900 p-6 shadow-2xl">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={form.name || ''} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full rounded-md border border-gray-700 bg-gray-950 px-3 py-2 text-sm text-white focus:border-cyan-500 focus:outline-none" />
                </div>
                <div>
                  <label className="mb-1 block text-xs text-gray-400">Category</label>
                  <input value={form.category || ''} onChange={(e) => setForm({ ...form, category: e.target.value })} className="w-full rounded-md border border-gray-700 bg-gray-950 px-3 py-2 text-sm text-white focus:border-cyan-500 focus:outline-none" />
                </div>
              </div>
              <div>
                <label className="mb-1 block text-xs text-gray-400">Description</label>
                <textarea value={form.description || ''} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={2} className="w-full rounded-md border border-gray-700 bg-gray-950 px-3 py-2 text-sm text-white focus:border-cyan-500 focus:outline-none" />
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="mb-1 block text-xs text-gray-400">Trigger</label>
                  <select value={form.trigger || 'manual'} onChange={(e) => setForm({ ...form, trigger: e.target.value })} className="w-full rounded-md border border-gray-700 bg-gray-950 px-3 py-2 text-sm text-white focus:border-cyan-500 focus:outline-none">
                ...
                <div>
                  ...
                  <select value={form.status || 'active'} onChange={(e) => setForm({ ...form, status: e.target.value as 'active' | 'disabled' })} className="w-full rounded-md border border-gray-700 bg-gray-950 px-3 py-2 text-sm text-white focus:border-cyan-500 focus:outline-none">
                ...
                <div>
                  ...
                  <input value={form.version || ''} onChange={(e) => setForm({ ...form, version: e.target.value })} className="w-full rounded-md border border-gray-700 bg-gray-950 px-3 py-2 text-sm text-white focus:border-cyan-500 focus:outline-none" />
                </div>
              </div>
              ...
              <div className="rounded-md border border-gray-800 bg-gray-950 p-3">
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <button onClick={addAction} className="rounded-md bg-cyan-600 px-2 py-1 text-xs text-white hover:bg-cyan-500">+ Add Action</button>
                </div>
                <div className="space-y-2">
                  {(form.actions || []).map((action, idx) => (
                    <div key={idx} className="grid grid-cols-12 gap-2 rounded-md border border-gray-800 bg-gray-900 p-2">
                      <div className="col-span-4"><input value={action.action} onChange={(e) => updateAction(idx, { ...action, action: e.target.value })} placeholder="action" className="w-full rounded border border-gray-700 bg-gray-950 px-2 py-1 text-xs text-white" /></div>
                      <div className="col-span-7"><input value={JSON.stringify(action.params)} onChange={(e) => { try { updateAction(idx, { ...action, params: JSON.parse(e.target.value) }) } catch { /* ignore malformed JSON while typing */ } }} placeholder='{"ip":"10.0.0.55"}' className="w-full rounded border border-gray-700 bg-gray-950 px-2 py-1 font-mono text-xs text-white" /></div>
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => setEditing(null)} className="rounded-md border border-gray-700 bg-gray-800 px-4 py-2 text-sm text-gray-300 hover:bg-gray-700">Cancel</button>
              <button onClick={save} disabled={!form.name || createMutation.isPending || updateMutation.isPending} className="rounded-md bg-cyan-600 px-4 py-2 text-sm text-white hover:bg-cyan-500 disabled:opacity-50">
